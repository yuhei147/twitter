...
if option == 'API認証とデータ取得':
    ...
    if uploaded_file is not None:
        ...
        st.write('認証が完了しました')
        username = st.text_input('ユーザー名を入力してください', 'SolaseedAir_JP')
        ...
        if st.button('データ取得'):
            ...
            data = []
            for tweet in tweepy.Paginator(client.get_users_tweets, user_id, exclude=exclude, tweet_fields=tweet_fields).flatten(limit=num_search_tweet):
                text, _id, public_metrics, created_at = tweet['text'], tweet[
                    'id'], tweet['public_metrics'], tweet['created_at']
                datum = [created_at, text, public_metrics['like_count'],
                         public_metrics['retweet_count'], _id]
                data.append(datum)
            ...
            message.success('csvファイルの取得が完了しました')
...
if option == 'データ分析':
    ...
    if uploaded_file is not None:
        ...
        hist = alt.Chart(df, title='いいね数の傾向').mark_bar().encode(
            ...
            alt.X('いいね', bin=alt.Bin(extent=[0, 400], step=10), title='いいね数'),
        ...
        result = pd.concat([base_time, mean, size],
                           axis=1).fillna(0)  # NANを0で埋める
        ...
        for grade in grades:
            ...
            txt = ' '.join(_df['ツイート本文'].to_list()).replace(
                'https://t.co/', '')  # スペースで結合
